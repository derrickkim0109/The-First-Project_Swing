...
public class MainMenu extends JDialog {
	...
	private JTextField tfMyPageIDtop;
...
//----------버튼 클릭 시 공지사항 목록으로 이동----------
	...
	private JPanel getInfoRevise() {
		if (infoRevise == null) {
			...
			infoRevise.add(getBtnSecession());
			...
			infoRevise.add(getTfPW());
			infoRevise.add(getTfPWCheck());
	...
	private JTextField getTfMypageSearch() {
		if (tfMypageSearch == null) {
			...
			tfMypageSearch.addFocusListener(new FocusAdapter() {
				@Override
				public void focusGained(FocusEvent e) {
					if(tfMypageSearch.getText().equals("검색어를 입력해주세요.")) {
						tfMypageSearch.setText("");
						tfMypageSearch.setForeground(Color.black);
					}
					else {
						tfMypageSearch.selectAll();
					}
				}
				@Override
				public void focusLost(FocusEvent e) {
					if(tfMypageSearch.getText().equals("")) {
						tfMypageSearch.setText("검색어를 입력해주세요.");
						tfMypageSearch.setForeground(Color.black);
					}
					else {
						tfMypageSearch.selectAll();
	...
	private JTextField getTfMyPageIDtop() {
		if (tfMyPageIDtop == null) {
			tfMyPageIDtop = new JTextField();
			tfMyPageIDtop.setBackground(UIManager.getColor("Button.background"));
			tfMyPageIDtop.setText((String) null);
			tfMyPageIDtop.setHorizontalAlignment(SwingConstants.TRAILING);
			tfMyPageIDtop.setColumns(10);
			tfMyPageIDtop.setBounds(302, 30, 89, 26);
	...
	private JTextField getTfEmail() {
		if (tfEmail == null) {
			...
			tfEmail.setBounds(75, 198, 90, 26);
	...
	private JPasswordField getTfPW() {
		if (tfPW == null) {
			...
			tfPW.setBounds(100, 119, 130, 26);
			tfPW.setText(ShareVar.pwUser);
		}
		return tfPW;
	}
	private JPasswordField getTfPWCheck() {
		if (tfPWCheck == null) {
			...
			tfPWCheck.setBounds(100, 148, 130, 26);
		}
		return tfPWCheck;
	}
	private JTextField getTfDomain() {
		if (tfDomain == null) {
			...
			tfDomain.setHorizontalAlignment(SwingConstants.LEFT);
			tfDomain.setText((String) null);
			tfDomain.setColumns(10);
			tfDomain.setBounds(189, 198, 90, 26);
	...
	private JLabel getLabel_3_1_1() {
		if (label_3_1 == null) {
			...
			label_3_1.setBounds(147, 203, 61, 16);
		}
		return label_3_1;
	}
	...
	private JButton getBtnSecession() {
		if (btnSecession == null) {
			...
			btnSecession.setBounds(222, 258, 78, 29);
		}
		return btnSecession;
	}
	...
	private JTextField getTfUserSearch() {
		if (tfUserSearch == null) {
			...
			tfUserSearch.addFocusListener(new FocusAdapter() {
				@Override
				public void focusGained(FocusEvent e) {
					if(tfUserSearch.getText().equals("검색어를 입력해주세요.")) {
						tfUserSearch.setText("");
						tfUserSearch.setForeground(Color.black);
					}
					else {
						tfUserSearch.selectAll();
					}
				}
				@Override
				public void focusLost(FocusEvent e) {
					if(tfUserSearch.getText().equals("")) {
						tfUserSearch.setText("검색어를 입력해주세요.");
						tfUserSearch.setForeground(Color.black);
					}
					else {
						tfUserSearch.selectAll();
			...
			tfUserSearch.setText("검색어를 입력해주세요.");
			tfUserSearch.setForeground(Color.LIGHT_GRAY);
...
//------------회원 조건 검색------------------	
	...
	private JTextField getTfNoitceSelect() {
		if (tfNoitceSelect == null) {
			...
			tfNoitceSelect.addFocusListener(new FocusAdapter() {
				@Override
				public void focusGained(FocusEvent e) {
					if(tfNoitceSelect.getText().equals("검색어를 입력해주세요.")) {
						tfNoitceSelect.setText("");
						tfNoitceSelect.setForeground(Color.black);
					}
					else {
						tfNoitceSelect.selectAll();
					}
				}
				@Override
				public void focusLost(FocusEvent e) {
					if(tfNoitceSelect.getText().equals("")) {
						tfNoitceSelect.setText("검색어를 입력해주세요.");
						tfNoitceSelect.setForeground(Color.black);
					}
					else {
						tfNoitceSelect.selectAll();
			...
			tfNoitceSelect.setText("검색어를 입력해주세요.");
			tfNoitceSelect.setForeground(Color.LIGHT_GRAY);
...
//-------- 회원관리 out table 추가 --------------
	...
	private JTextField getTfUserNoticeSearch() {
		if (tfUserNoticeSearch == null) {
			...
			tfUserNoticeSearch.addFocusListener(new FocusAdapter() {
				@Override
				public void focusGained(FocusEvent e) {
					if(tfUserNoticeSearch.getText().equals("검색어를 입력해주세요.")) {
						tfUserNoticeSearch.setText("");
						tfUserNoticeSearch.setForeground(Color.black);
					}
					else {
						tfUserNoticeSearch.selectAll();
					}
				}
				@Override
				public void focusLost(FocusEvent e) {
			...
			tfUserNoticeSearch.setText("검색어를 입력해주세요.");
			tfUserNoticeSearch.setForeground(Color.LIGHT_GRAY);
...
//----inner table 조건 추가-----------
	...
	private JComboBox getCbEmail() {
		if (cbEmail == null) {
			...
			cbEmail.setBounds(279, 199, 107, 27);
		}
		return cbEmail;
	}
...
//	    	tfPWCheck.requestFocus();
	...
	private JLabel getLblNewLabel_2() {
		if (lblNewLabel_2 == null) {
			...
			ImageIcon icon = new ImageIcon("/Users/tj/Desktop/KakaoTalk_Photo_2020-11-13-18-12-00.png"); 
	        Image img = icon.getImage();
	        Image changeImage = img.getScaledInstance(438, 460, Image.SCALE_DEFAULT);
	        ImageIcon changeIcon = new ImageIcon(changeImage);
	        lblNewLabel_2.setIcon(new ImageIcon(changeImage));
