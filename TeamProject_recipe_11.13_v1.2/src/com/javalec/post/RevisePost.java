...
public class RevisePost extends JDialog {
	...
	private int writeReviseAction() {
		...
		dbActionPost.writeReviseAction(cbMenu.getSelectedItem().toString(), cbCookTime.getSelectedIndex()+1, tfTitle.getText().trim(),textArea.getText().trim(),input);
	...
	private void GetText() {
		...
		System.out.println(ShareVar.cbmenu);
		System.out.println(ShareVar.cbcooktime);
	...
	private void commentUpdateAction() { // id 체크후 수정창으로 이동
		...
		if(commentIdCheck.equals(ShareVar.idUser)||ShareVar.userno.equals("1")) { // 댓글작성자, admin만 수정가능
			...
			JOptionPane.showMessageDialog(null, "본인이 작성한 댓글만 수정가능");
		}
	}
	...
	private void commentDeleteAction() {
		...
		if(commentIdCheck.equals(ShareVar.idUser)||ShareVar.userno.equals("1")) {
			...
			JOptionPane.showMessageDialog(null, "본인이 작성한 댓글만 삭제 가능");
		}
...
//				btnLike.setBackground(new Color(255,255,255));
	...
	private JLabel getLbllike() {
		if (lbllike == null) {
			...
			ImageIcon icon = new ImageIcon("/Users/tj/Downloads/cutlery-2.png"); 
	        Image img = icon.getImage();
	        Image changeImage = img.getScaledInstance(70, 59, Image.SCALE_DEFAULT);
	        ImageIcon changeIcon = new ImageIcon(changeImage);
	        lbllike.setIcon(new ImageIcon(changeImage));
